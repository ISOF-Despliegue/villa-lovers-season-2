    ...
    <div className="auth-shell">
      ...
      <div className="auth-card">
        ...
        <div className="form-group">
          ...
          <input type="password" placeholder="Enter your password" value={pass} onChange={e => setPass(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()} />
        </div>
        {err && <div style={{ fontSize: 13, color: "var(--danger)", marginBottom: 12 }}>{err}</div>}
        <div className="forgot-row"><span className="auth-link" style={{ fontSize: 13 }}>¿Olvidaste tu contraseña?</span></div>

        <button className="btn-primary" style={{ width: "100%", marginBottom: 16 }} onClick={handleLogin}>Iniciar Sesión</button>
        <div className="form-divider"><span>o</span></div>
        <button className="btn-ghost" style={{ width: "100%", marginBottom: 16 }}>Continuar con Google</button>
        <div className="auth-footer">¿No tienes cuenta? <span className="auth-link" onClick={onRegister}>Regístrate</span></div>
      </div>
    </div>
  );
}
...
export function RegisterPage({ onLogin, onBack }) {
  const [form, setForm] = useState({ email: "", username: "", password: "", confirm: "", role: "listener" });
  const [err, setErr] = useState("");
      ...
      <div className="auth-card">
        ...
        <div className="role-tabs" style={{ marginBottom: 20 }}>
          {["listener","artist"].map(r => (
            <div key={r} className={`role-tab${form.role === r ? " active" : ""}`} onClick={() => setForm(f => ({ ...f, role: r }))}>
        ...
        {["email","username","password","confirm"].map((k, i) => (
          <div className="form-group" key={k}>
            <label className="form-label">{["Email","Nombre de usuario","Contraseña","Confirmar contraseña"][i]}</label>
            <input type={k.includes("pass") || k === "confirm" ? "password" : "text"} placeholder={["Enter your email","Choose a username","Create a password","Confirm your password"][i]} value={form[k]} onChange={set(k)} />
          </div>
        ))}

        {err && <div style={{ fontSize: 13, color: "var(--danger)", marginBottom: 12 }}>{err}</div>}
        <button className="btn-primary" style={{ width: "100%", marginBottom: 16 }} onClick={handleCreate}>Crear Cuenta</button>
        <div className="form-divider"><span>o</span></div>
        <button className="btn-ghost" style={{ width: "100%", marginBottom: 16 }}>Registrarse con Google</button>
        <div className="auth-footer">¿Ya tienes cuenta? <span className="auth-link" onClick={onBack}>Inicia sesión</span></div>
      </div>
    </div>
  );
}